...
func main() {
	...
	initial_pos := setup_initial_positions()
	positions := setup_initial_positions()

	// find first time the pattern repeats
	repeats_at := -1
	for i := 1; i <= 1000000000; i++ {
		perform_dance(positions, instructions)
		if reflect.DeepEqual(positions,initial_pos) {
			repeats_at = i
			break
		}
	}

	// So our pattern should look like the (1000000000 mod 60)th iteration
	positions = setup_initial_positions()
	for i := 1; i <= (1000000000 % repeats_at); i++ {
		perform_dance(positions, instructions)
	}

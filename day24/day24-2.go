...
func connect_next(ports []Port, w int, sum int, length int) {
	found_next := false
	for i, v := range(ports) {
		if v.side1 == w || v.side2 == w {
			...
			if v.side1 == w {
				connect_next(next_ports, v.side2, new_sum,length+1)
			} else {
				connect_next(next_ports, v.side1, new_sum,length+1)
			}
		} else {
			continue
		}
	}
	if !found_next && length >= max_length && sum > max_weight {
		max_weight = sum
		max_length = length
	}
}
...
func build_bridges(ports []Port) {
	...
	for _, v := range(starting_points) {
		...
		if ports[v].side1 == 0 {
			...
			connect_next(remaining_ports,next_weight,next_weight,1)

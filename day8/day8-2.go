...
func process_instructions(f string) int {
	file, _ := os.Open(f)
	s := bufio.NewScanner(file)
	...
	for s.Scan() {
		...
		if matches != nil {
			...
			if test_instruction(op_comp_reg, Comparator(op_comp), op_comp_val) {
				...
				if registers[op_reg] > largest_val {
					largest_val = registers[op_reg]
				}
			}
		} else {
...
func main() {
	...
	largest_in_proc := process_instructions(datafile)
	fmt.Printf("Largest ever value: %d", largest_in_proc)
}
